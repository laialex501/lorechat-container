...
from app.services.prompts import BasePrompt, PersonaType, PromptFactory
...
def retrieve_context(state: ChatState, vector_store: BaseVectorStoreService) -> Dict[str, Any]:
    ...
    # Search vector store
    docs = vector_store.as_retriever().invoke(
        latest_message.content
    )
    return {"retrieved_docs": docs}


def generate_response(state: ChatState, llm_service: BaseLLMService, prompt: BasePrompt) -> Dict[str, Any]:
